  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="container px-3 sm:px-6 mx-auto py-4 sm:py-6 max-w-7xl">
        {/* Dashboard Header */}
        <header className="mb-4 sm:mb-8 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-lg shadow-md p-4 sm:p-6 text-white">
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Dashboard</h1>
          <p className="mt-1 sm:mt-2 text-sm sm:text-base text-blue-100">Manage your career documents and track your application progress</p>
        </header>

        {/* Profile Status Alert */}
        {(hasCVUploaded || hasLinkedInConnected) && (
          <Alert className="mb-4 sm:mb-8 bg-blue-50 border-blue-200 shadow-sm text-xs sm:text-sm">
            <div className="flex items-center">
              <CheckCircle2 className="h-4 sm:h-5 w-4 sm:w-5 text-blue-500 mr-1 sm:mr-2 flex-shrink-0" />
              <AlertDescription className="text-blue-700 py-1">
        ...
        <section className="mb-4 sm:mb-8">
          <Card className="border-t-4 border-t-primary shadow-md overflow-hidden">
            <CardHeader className="bg-gray-50 border-b text-center py-2 sm:py-4">
                ...
                <div className="w-full max-w-md">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button className="w-full h-auto py-2 sm:py-3 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white transition-all shadow-sm hover:shadow-md text-sm sm:text-base">
                    ...
                    <DropdownMenuContent align="center" className="w-48 sm:w-56">
                      <Link href="/dashboard/cover-letters?tab=create" className="block w-full">
                        <DropdownMenuItem className="cursor-pointer py-2 sm:py-3 flex items-center text-xs sm:text-sm">
                          <FileText className="h-3 w-3 sm:h-4 sm:w-4 mr-2 text-blue-500" />
                          <span>Cover Letter</span>
                        </DropdownMenuItem>
                      </Link>
                      <Link href="/dashboard/resumes/new" className="block w-full">
                        <DropdownMenuItem className="cursor-pointer py-2 sm:py-3 flex items-center text-xs sm:text-sm">
                          <FileBadge className="h-3 w-3 sm:h-4 sm:w-4 mr-2 text-green-500" />
                          <span>Resume</span>
                        </DropdownMenuItem>
                      </Link>
                    </DropdownMenuContent>
              ...
              {loadingLetters ? (
                ...
                <div className="space-y-2">
                  {recentLetters.map((letter) => (
                    <div key={letter.id} className="p-2 sm:p-3 rounded-md hover:bg-blue-50 transition-colors border border-gray-100">
                      <div className="flex items-start justify-between gap-1 sm:gap-2">
                        <div className="flex items-start flex-1 min-w-0">
                          <FileText className="h-4 w-4 sm:h-5 sm:w-5 text-blue-500 mt-0.5 mr-1 sm:mr-2 flex-shrink-0" />
                ...
                <div className="text-center py-4 sm:py-8 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                  <FileText className="h-8 w-8 sm:h-10 sm:w-10 text-gray-400 mx-auto mb-1 sm:mb-2" />
                  <h3 className="text-xs sm:text-sm font-medium mb-1">No cover letters yet</h3>
                  <Link href="/dashboard/cover-letters?tab=create">
                    <Button size="sm" className="mt-1 sm:mt-2 text-xs h-7">
              ...
              {loadingResumes ? (
                ...
                <div className="space-y-2">
                  {recentResumes.map((resume) => (
                    <div key={resume.id} className="p-2 sm:p-3 rounded-md hover:bg-green-50 transition-colors border border-gray-100">
                      <div className="flex items-start justify-between gap-1 sm:gap-2">
                        <div className="flex items-start flex-1 min-w-0">
                          <FileBadge className="h-4 w-4 sm:h-5 sm:w-5 text-green-500 mt-0.5 mr-1 sm:mr-2 flex-shrink-0" />
                          <div className="min-w-0">
                ...
                <div className="text-center py-4 sm:py-8 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                  <FileBadge className="h-8 w-8 sm:h-10 sm:w-10 text-gray-400 mx-auto mb-1 sm:mb-2" />
            ...
            <CardContent className="p-3 sm:p-4">
              <div className="flex justify-center">
                <div className="grid grid-cols-2 gap-3 sm:gap-6 w-full max-w-md">
                  <div className="bg-blue-50 border border-blue-100 p-3 sm:p-6 rounded-lg text-center transition-transform hover:shadow-md">
                    <p className="text-xl sm:text-3xl font-bold text-blue-600">{stats.totalLetters}</p>
                    <p className="text-xs sm:text-sm text-gray-600 mt-0 sm:mt-1">Total Letters</p>
                  </div>
                  <div className="bg-green-50 border border-green-100 p-3 sm:p-6 rounded-lg text-center transition-transform hover:shadow-md">
                    <p className="text-xl sm:text-3xl font-bold text-green-600">{recentResumes.length}</p>
                    <p className="text-xs sm:text-sm text-gray-600 mt-0 sm:mt-1">Total Resumes</p>
                  </div>
                </div>
              </div>
            </CardContent>
