        ...
        {(resumes.length > 0 || !isLoading) && (
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            ...
            >
              {importLoading ? (
                <>
                  <LoadingSpinner className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" /> Importing...
                </>
              ) : (
                <>
                  <Upload className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" /> Import Resume
                </>
              )}
            </Button>
          </div>
      ...
      <div className="flex flex-col xs:flex-row justify-between items-start xs:items-center gap-4">
        ...
        <div className="w-full xs:w-auto relative">
          ...
          <Input 
            ...
            className="pl-7 sm:pl-8 w-full xs:w-[200px] sm:w-[250px] h-9 text-sm focus-visible:ring-orange-500" 
          />
        </div>
      </div>
      ...
      {filteredResumes.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-6">
          {filteredResumes.map((resume) => (
            <Card 
              key={resume.id} 
              className={`overflow-hidden hover:border-orange-300 transition-colors ${resume.id === importedResumeId ? 'border-orange-500 shadow-md ring-1 ring-orange-500' : ''}`}
            >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      ...
                      <DropdownMenuItem onClick={() => router.push(`/dashboard/resumes/${resume.id}`)}> 
                        <Edit className="h-4 w-4 mr-2 text-orange-600" /> Edit 
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => router.push(`/dashboard/resumes/${resume.id}/tailor`)}> 
                        <Target className="h-4 w-4 mr-2 text-orange-600" /> Tailor for Job 
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => router.push(`/dashboard/resumes/${resume.id}/preview`)}> 
                        <FileText className="h-4 w-4 mr-2 text-orange-600" /> Preview 
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => router.push(`/dashboard/resumes/${resume.id}/ats-scanner`)}> 
                        <FileSearch className="h-4 w-4 mr-2 text-orange-600" /> ATS Scanner 
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleDuplicate(resume.id)}> 
                        <Copy className="h-4 w-4 mr-2 text-orange-600" /> Duplicate 
                      </DropdownMenuItem>
                      {resume.sourceCV && (
              ...
              <CardFooter className="p-4 pt-2 flex gap-2">
                <Button 
                  variant="outline" 
                  size="sm" 
                  className="flex-1 text-xs h-8 px-2 sm:px-3 border-orange-200 text-orange-700 hover:bg-orange-50" 
                ...
                > 
                  <Target className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" /> Tailor 
                </Button>
                <Button 
                  size="sm" 
                  className="flex-1 text-xs h-8 px-2 sm:px-3 bg-orange-600 hover:bg-orange-700" 
