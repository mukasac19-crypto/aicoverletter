...
export default function ResumeDashboardPage() {
  ...
  // Load resumes from database (client-side read)
  useEffect(() => {
  ...
  // --- UPDATED handleFileSelect function (Import - Uses API) ---
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // --- CV Record Creation REMOVED from client-side ---
      // Implement server-side CV record creation if needed (e.g., in parser or dedicated API)

      // Prepare form data for the parser API
      const formData = new FormData();
      formData.append('file', file);

      console.log(`Calling parser API for file: ${file.name}`);
      const parseResponse = await fetch('/api/resumes/parser', { method: 'POST', body: formData });
      if (!parseResponse.ok) { const errData = await parseResponse.json(); throw new Error(errData.error || `Parse failed (${parseResponse.status})`); }
      ...
      const newResumePayload: Partial<ResumeData> = {
        ...
        isPublic: false,
        is_imported: true,
        sourceCV: parsedData.sourceCV || null, // Use camelCase key from parser if available
      };

      console.log("Creating imported resume via POST API. Payload:", newResumePayload);

      // Call the POST API route to create the new resume
      const createResponse = await fetch('/api/resumes', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ resumeData: newResumePayload }), // Wrap for POST API
      });
      const result = await createResponse.json(); // API returns created record (snake_case)
      if (!createResponse.ok) throw new Error(result.error || `Failed to save imported resume (${createResponse.status})`);
      if (!result?.id) throw new Error('Failed to save imported resume (API did not return created record).');

      // Map response back for UI state
      const newlyCreatedResume = mapDatabaseToResumeData(result as unknown as DatabaseResumeData);
      if (!newlyCreatedResume) throw new Error("Failed to process imported resume data from server.");

      // Update local state
      setResumes(prevResumes => [newlyCreatedResume, ...prevResumes]);
      setImportedResumeId(newlyCreatedResume.id);
      setShowImportGuide(true);

      toast({ title: "Resume Imported & Saved", description: "Resume imported and saved successfully!" });
  ...
  // --- END OF UPDATED handleFileSelect ---
