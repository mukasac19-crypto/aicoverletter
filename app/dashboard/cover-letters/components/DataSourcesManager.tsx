//C:\Users\mukas\Downloads\project-bolt-sb1-guerg2d9\project\app\dashboard\cover-letters\components\DataSourcesManager.tsx
"use client";
...
const DataSourcesManager: React.FC<DataSourcesManagerProps> = ({
  ...
  // Find the selected CV once to avoid repetition and fix the error
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          ...
          <div
            className={`flex items-center rounded-md border p-3 ${
              hasCV ? "border-green-500/50 bg-green-500/10" : "border-muted bg-muted/50"
            } cursor-pointer`}
            ...
            <div>
              ...
              {hasCV && selectedCV ? (
                <p className="text-xs text-green-600">
                  {selectedCV.name.length > 15
                    ? `${selectedCV.name.substring(0, 15)}...`
                    : selectedCV.name}
                </p>
              ) : (
                <p className="text-xs text-blue-600 font-medium hover:underline">Connect</p>
              )}
            </div>
          </div>
          ...
          <div
            className={`flex items-center rounded-md border p-3 ${
              hasLinkedIn ? "border-green-500/50 bg-green-500/10" : "border-muted bg-muted/50"
            } cursor-pointer`}
            onClick={() => setIsOpen(true)}
          >
