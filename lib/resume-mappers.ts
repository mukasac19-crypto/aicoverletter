...
export function mapDatabaseToResumeData(dbData: any): ResumeData | null {
  if (!dbData) {
  ...
  try {
    ...
    const mappedData: ResumeData = {
      ...
      // Add support for joined template data
      resume_templates: dbData.resume_templates || undefined,
    };
...
// IMPORTANT: Return type is now 'any' to ensure compatibility with Supabase's Json type
export function mapResumeToDatabase(uiData: ResumeData | null | undefined): any {
  if (!uiData) {
  ...
  try {
    const mappedData: any = {
      ...
      title: uiData.title,
      personal_info: uiData.personalInfo as any,
      work_experience: uiData.workExperience as any,
      education: uiData.education as any,
      skills: uiData.skills as any,

      // Map optional UI fields to null for database (Supabase expects null, not undefined)
      projects: (Array.isArray(uiData.projects) && uiData.projects.length > 0) ? uiData.projects as any : null,
      languages: (Array.isArray(uiData.languages) && uiData.languages.length > 0) ? uiData.languages as any : null,
      certifications: (Array.isArray(uiData.certifications) && uiData.certifications.length > 0) ? uiData.certifications as any : null,
      interests: (Array.isArray(uiData.interests) && uiData.interests.length > 0) ? uiData.interests as any : null,
      internships: (Array.isArray(uiData.internships) && uiData.internships.length > 0) ? uiData.internships as any : null,
      references: (Array.isArray(uiData.references) && uiData.references.length > 0) ? uiData.references as any : null,
      custom_sections: (Array.isArray(uiData.customSections) && uiData.customSections.length > 0) ? uiData.customSections as any : null,

      reference_text: uiData.referenceText || null, // Map empty/undefined to null

      // FIX: template_id is required string in DatabaseResumeData. Assume uiData.templateId is valid.
      template_id: uiData.templateId,

      is_public: uiData.isPublic ?? false,

      // FIX: source_cv is optional string in DatabaseResumeData. Map empty/undefined to null.
      source_cv: uiData.sourceCV || null,

      updated_at: uiData.updated_at || new Date().toISOString(), // Ensure updated_at is set
      // Map other fields if they exist in both types
      is_imported: uiData.is_imported ?? null,
